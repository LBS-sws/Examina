...
class ChapterClassModel extends CFormModel
{
    public $id;
    public $menu_id;
    public $menu_name;
    ...
    public function rules()
    {
        return array(
            ...
            array('id','validateID','on'=>array("edit","delete")),
            array('id','validateDel','on'=>array("delete")),
            array('z_index,random_num', 'numerical', 'min'=>0, 'integerOnly'=>true),
        );
    }
    ...
    public function validateID($attribute, $params){
        $row = Yii::app()->db->createCommand()->select("a.menu_id,b.menu_code")
    ...
    public function echoMedia(){
        ...
        if(!empty($this->class_list)){
            ...
            foreach ($this->class_list as $row){
                $i++;
                //章节修改
                $linkEdit = Yii::app()->createUrl('ChapterClass/edit',array("index"=>$row["id"]));
                //章节练习
                ...
                $html.='<div class="media-left media-middle"><span class="num_i chapter">'.$i.'</span></div>';
                $html.=TbHtml::link("<h4>{$row["chapter_name"]}</h4>",$linkText,array("class"=>"media-body media-middle"));
                $html.='<div class="media-right media-middle">';
                $html.='<div class="chapter-link-div">';
